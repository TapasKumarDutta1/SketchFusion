...
from utils.utils_SD_CLIP import permute_indices, flip_keypoints


def get_corr_map_loss(
    img1_desc,
    img2_desc,
    corr_map_net,
    img1_patch_idx,
...
def self_contrastive_loss(feat_map, instance_mask=None):
    ...
    if instance_mask is not None:
        ...
        instance_mask = (
            F.interpolate(
                instance_mask.cuda().unsqueeze(1).float(), size=(H, W), mode="bilinear"
            )
            > 0.5
        )
    ...
    if instance_mask is not None:
        mask = (
            mask & instance_mask[0, 0, idx_i, idx_j] & instance_mask[0, 0, idx_k, idx_l]
        )
...
def cal_clip_loss(image_features, text_features, logit_scale, self_logit_scale=None):
    ...
    logits_per_image, logits_per_text = get_logits(
...
def calculate_patch_indices_and_loss(
    args,
    kps_1,
    kps_2,
    desc_1,
...
def calculate_loss(
    ...
    if args.DENSE_OBJ > 0:  # dense training objective loss
        ...
        if args.GAUSSIAN_AUGMENT > 0:
            std = (
                args.GAUSSIAN_AUGMENT * img2_threshold / 2
            )  # 2 sigma within the threshold
            noise = torch.randn_like(gt_flow, dtype=torch.float32) * std
    ...
    if (
        args.ADAPT_FLIP > 0
        or args.AUGMENT_SELF_FLIP > 0
        or args.AUGMENT_DOUBLE_FLIP > 0
    ):  # augment with flip
        loss = [loss]
        loss_weight = [1]
        ...
        if args.ADAPT_FLIP > 0:
            vis_flip = (
                img1_kps_flip[:, 2] * img2_kps[:, 2] > 0
            )  # mutual visibility after flip
            if vis_flip.sum() > 0:
                loss_flip = calculate_patch_indices_and_loss(
                    args,
                    img1_kps_flip[vis_flip],
                    img2_kps[vis_flip],
                    img1_desc_flip,
                    img2_desc,
                    scale_factor,
        ...
        if args.AUGMENT_DOUBLE_FLIP > 0:
            vis_double_flip = (
                img1_kps_flip[:, 2] * img2_kps_flip[:, 2] > 0
            )  # mutual visibility after flip
            if vis_double_flip.sum() > 0:
                loss_double_flip = calculate_patch_indices_and_loss(
                    args,
                    img1_kps_flip[vis_double_flip],
                    img2_kps_flip[vis_double_flip],
                    img1_desc_flip,
                    img2_desc_flip,
                    scale_factor,
                    num_patches,
        ...
        if args.AUGMENT_SELF_FLIP > 0:
            ...
            if vis_self_flip.sum() > 0:
                loss_self_flip = calculate_patch_indices_and_loss(
                    args,
                    img1_kps_flip[vis_self_flip],
                    img1_kps[vis_self_flip],
                    img1_desc_flip,
                    img1_desc,
                    scale_factor,
                    num_patches,
    ...
    if args.SELF_CONTRAST_WEIGHT > 0:
        contrast_loss1 = (
            self_contrastive_loss(
                img1_desc.permute(0, 2, 1).reshape(
                    -1, args.PROJ_DIM, num_patches, num_patches
                ),
                mask1.unsqueeze(0),
            )
        ...
        contrast_loss2 = (
            self_contrastive_loss(
                img2_desc.permute(0, 2, 1).reshape(
                    -1, args.PROJ_DIM, num_patches, num_patches
                ),
                mask2.unsqueeze(0),
            )
        ...
        contrast_loss = (
            (contrast_loss1 + contrast_loss2) / 2 * args.SELF_CONTRAST_WEIGHT
        )
        loss += contrast_loss
